...
def model_simulation(inputs):
	...
	model_levee_PI = inputs[6]

	exo_values = dict()
	# 1. initialise the model
	exo_values = exo_value_def(exo_values)
	model_levee_output = \
		model_levee_PI.run(params={'FINAL TIME':interval_tick,
	...
	for n in range(run_time_till_now - 1):
		...
		# todo - ERC not implemented as it is a lookup at the moment [everywhere in model this problem]
		model_levee_output_inter = \
			model_levee_PI.run(params={'aging time':exo_values['AT_value'],
							   ...
							   return_timestamps=np.linspace(1 + n,
															 1 + n + 1,
															 1//time_step_SD))
	...
	model_levee_output_inter2 = \
		model_levee_PI.run(params={'aging time':exo_values['AT_value'],
						   ...
						   return_timestamps=np.linspace(1 + run_time_till_now,
														 1 + run_time_till_now + policy_eval_tick,
														 1 // time_step_SD))
...
def policy_impact_evaluation(model_run_PE, KPIs_init, policy_eval_tick,
							 policy_chosen_history, run_time_till_now, time_step_SD, interval_tick):
	...
	for j in range(len(policy_ins)):
		...
		# intermediate.append(pysd.read_vensim('Flood_Levees_14_Final.py'))
