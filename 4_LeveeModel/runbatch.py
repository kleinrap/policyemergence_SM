		...
		for rep_runs in range(repetitions_runs):
			...
			if exp_i == 1 and sce_i == 1 and rep_runs >= 0:
				...
				for i in range(run_tick):
					...
					policy_impact_evaluation(model_run_SM, KPIs, policy_eval_tick, policy_chosen_history,
											 i + 1, time_step_SD, interval_tick, stocks, exo_values, sce_i)

					# running the policy emergence model
					policy_chosen = model_run_SM.step(KPIs)
					policy_chosen_history.append(policy_chosen)
					...
					model_levee_output_inter = \
						model_levee.run(params={'aging time':exo_values['AT_value'],
										...
										return_timestamps=np.linspace(1 + i, 1 + i + 1, 1 // time_step_SD))
				...
				model_levee_output.to_csv('O_L1_model_Sce' + str(sce_i) + '_Exp' + str(exp_i) + '_Run' + str(rep_runs) + '.csv')

				# policy emergence model
				output_SM_model = model_run_SM.datacollector.get_model_vars_dataframe()
				output_SM_model.to_csv('O_PE_model_Sce' + str(sce_i) + '_Exp' + str(exp_i) + '_Run' + str(rep_runs) + '.csv')
				output_SM_agents = model_run_SM.datacollector.get_agent_vars_dataframe()
				output_SM_agents.to_csv('O_PE_agents_Sce' + str(sce_i) + '_Exp' + str(exp_i) + '_Run' + str(rep_runs) + '.csv')