...
def model_simulation(inputs):
	...
	2. run for the duration that has already run for with the policies already implemented
	3. run one additional round for the new test policy
	'''

	policy_chosen = inputs[0] # policy chosen
	policy_eval_tick = inputs[1]
	policy_chosen_history = inputs[2]
	run_time_till_now = inputs[3]
	time_step_SD = inputs[4]
	interval_tick = inputs[5]

	model_levee_PI_test = pysd.read_vensim('Flood_Levees_14_Final.mdl')

	exo_values = dict()
	# 1. initialise the model
	exo_values = exo_value_def(exo_values)
	model_levee_output = model_levee_PI_test.run(params={'FINAL TIME':interval_tick, 'aging time':exo_values['AT_value'], 'obsolescence time':exo_values['OT_value'], 'design time':exo_values['DT_value'], 'flood perception time':exo_values['FPT_value'], 'renovation time':exo_values['RT_value'], 'adjustment time':exo_values['AdT_value'], 'planning horizon':exo_values['PH_value'], 'renovation standard':exo_values['RS_value'], 'construction time':exo_values['CT_value']})

	# 2. Advancing as a copy
	for n in range(run_time_till_now - 1):
		# introduce the change in policy here
		exo_values = policy_implementation(policy_chosen_history[n], exo_values)
		# todo - ERC not implemented as it is a lookup at the moment
		model_levee_output_inter = model_levee_PI_test.run(params={'aging time':exo_values['AT_value'], 'obsolescence time':exo_values['OT_value'], 'design time':exo_values['DT_value'], 'flood perception time':exo_values['FPT_value'], 'renovation time':exo_values['RT_value'], 'adjustment time':exo_values['AdT_value'], 'planning horizon':exo_values['PH_value'], 'renovation standard':exo_values['RS_value'], 'construction time':exo_values['CT_value']}, initial_condition='current', return_timestamps=np.linspace(1+n,1+n+1, 1//time_step_SD))

	# 3. Testing the policy
	exo_values = policy_implementation(policy_chosen, exo_values)
	for n in range(policy_eval_tick):
		model_levee_output_inter2 = model_levee_PI_test.run(params={'aging time':exo_values['AT_value'], 'obsolescence time':exo_values['OT_value'], 'design time':exo_values['DT_value'], 'flood perception time':exo_values['FPT_value'], 'renovation time':exo_values['RT_value'], 'adjustment time':exo_values['AdT_value'], 'planning horizon':exo_values['PH_value'], 'renovation standard':exo_values['RS_value'], 'construction time':exo_values['CT_value']}, initial_condition='current', return_timestamps=np.linspace(1+run_time_till_now+n,1+run_time_till_now+n+1, 1//time_step_SD))

	KPIs = states_calculation(model_levee_PI_test) # mapping the states

	return KPIs

def policy_impact_evaluation(model_run_PE, model_run_context, KPIs_init, policy_eval_tick, policy_chosen_history, run_time_till_now, time_step_SD, interval_tick):
	...
	for j in range(len(policy_ins)):
		...
		intermediate.append(policy_chosen_history)
	...
	results = []
	for j in range(len(policy_ins)):
		results_t = model_simulation(inputs[j])
