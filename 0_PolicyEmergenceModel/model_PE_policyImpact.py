...
def model_simulation(inputs):
	...
	model_run_context_PI_test = inputs[2] # inputs for the policy context model
...
def policy_impact_evaluation(model_run_PE, model_run_context, KPIs_init, interval_tick):
	...
	model_run_context_PI_test = copy.deepcopy(model_run_context) # copy of the model in its current state
	...
	for j in range(len(policy_ins)):
		...
		intermediate.append(model_run_context_PI_test)
